...
const reviewerToken = () => `Bearer ${signToken({ userId: "rv-1", email: "reviewer@test.com", role: "reviewer" })}`;
beforeEach(() => {
    vi.clearAllMocks();
    prisma.processingJob.count.mockResolvedValue(5);
});
describe("GET /api/jobs", () => {
    ...
    it("returns job list for admin", async () => {
        ...
        prisma.processingJob.findMany.mockResolvedValueOnce([job]);
    ...
    it("supports status filter", async () => {
        ...
        expect(res.status).toBe(200);
        const call = prisma.processingJob.findMany.mock.calls[0][0];
