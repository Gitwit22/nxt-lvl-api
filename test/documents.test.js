...
// Token helpers
const uploaderToken = () => `Bearer ${signToken({ userId: "user-uploader", email: "uploader@test.com", role: "uploader" })}`;
const reviewerToken = () => `Bearer ${signToken({ userId: "user-reviewer", email: "reviewer@test.com", role: "reviewer" })}`;
const adminToken = () => `Bearer ${signToken({ userId: "user-admin", email: "admin@test.com", role: "admin" })}`;
beforeEach(() => {
    vi.clearAllMocks();
    ...
    prisma.document.findUniqueOrThrow.mockResolvedValue(doc);
