...
const TIMEFLOW_APP_URL = (process.env.TIMEFLOW_APP_URL ?? process.env.FRONTEND_BASE_URL ?? "https://timeflow.nltops.com").replace(/\/$/, "");

export interface TimeflowTeamInviteEmailParams {
  to: string;
  recipientName: string;
  organizationName: string;
  role: string;
...
export async function sendTimeflowTeamInviteEmail(params: TimeflowTeamInviteEmailParams): Promise<boolean> {
  const service = getEmailService();
  if (!service) {
    logger.warn("[email] Email service unavailable — skipping TimeFlow team invite email", { to: params.to });
    return false;
  }

  const link = `${TIMEFLOW_APP_URL}/accept-invite?token=${encodeURIComponent(params.rawToken)}`;
  const expiryDate = params.expiresAt.toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" });
  const subject = `You've been invited to join ${params.organizationName} on Time Flow`;
  const html = buildTimeflowInviteHtml({ ...params, link, expiryDate });
  const text = buildTimeflowInviteText({ ...params, link, expiryDate });

  const result = await service.execute({ to: params.to, subject, html, text });

  if (result.success) {
    logger.info("[email] TimeFlow team invite sent", { to: params.to });
    return true;
  }

  logger.warn("[email] TimeFlow team invite email failed", { to: params.to });
  return false;
}

function buildTimeflowInviteText(params: TimeflowTeamInviteEmailParams & { link: string; expiryDate: string }): string {
  const { recipientName, organizationName, role, link, expiryDate } = params;
  return [
    `Hi ${recipientName},`,
    "",
    `You were invited to join the ${organizationName} workspace in Time Flow.`,
    `Role: ${role}`,
    "",
...
function buildTimeflowInviteHtml(params: TimeflowTeamInviteEmailParams & { link: string; expiryDate: string }): string {
  const { recipientName, organizationName, role, link, expiryDate } = params;
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#f4f4f5;font-family:sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f5;padding:40px 0;">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
        <tr><td style="background:#18181b;padding:28px 40px;">
          <h1 style="margin:0;color:#ffffff;font-size:20px;font-weight:700;">Nxt Lvl — Time Flow</h1>
        </td></tr>
        <tr><td style="padding:36px 40px;">
          <p style="margin:0 0 16px;font-size:16px;color:#18181b;">Hi ${recipientName},</p>
          <p style="margin:0 0 24px;font-size:15px;color:#444;line-height:1.6;">
            You were invited to join the <strong>${organizationName}</strong> workspace in Time Flow.
          </p>
          <div style="background:#f9f9fb;border-radius:6px;padding:16px 20px;margin:0 0 28px;">
            <p style="margin:4px 0;color:#555;">Role: <strong style="color:#18181b;">${role}</strong></p>
          </div>
          <table cellpadding="0" cellspacing="0" style="margin:0 0 28px;">
            <tr><td style="background:#18181b;border-radius:6px;padding:14px 28px;">
              <a href="${link}" style="color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;">Accept Invite</a>
            </td></tr>
          </table>
          <p style="margin:0 0 8px;font-size:12px;color:#888;">Or copy this link into your browser:</p>
          <p style="margin:0 0 16px;font-size:12px;color:#555;word-break:break-all;">${link}</p>
          <p style="margin:0 0 32px;font-size:13px;color:#888;">This link expires on <strong>${expiryDate}</strong>.</p>
          <hr style="border:none;border-top:1px solid #e5e7eb;margin:0 0 24px;">
          <p style="margin:0;font-size:12px;color:#999;line-height:1.6;">
...
export async function sendEventurePersonnelInviteEmail(params: EventurePersonnelInviteEmailParams): Promise<boolean> {
  const service = getEmailService();
  if (!service) {
    logger.warn("[email] Email service unavailable — skipping Eventure personnel invite email", { to: params.to });
    return false;
  }
  ...
  if (result.success) {
    logger.info("[email] Eventure personnel invite sent", { to: params.to });
    return true;
  }

  if ("skipped" in result && result.skipped) {
    logger.warn("[email] Eventure invite email skipped (sending disabled)", { to: params.to });
  } else {
    logger.warn("[email] Eventure invite email failed", {
      to: params.to,
      code: "error" in result ? (result as { success: false; error: { code: string } }).error?.code : undefined,
    });
  }
...
function buildEventureInviteText(params: EventurePersonnelInviteEmailParams & { link: string }): string {
  const { recipientName, organizationName, assignedRole, link } = params;
  return [
    `Hi ${recipientName},`,
    "",
    `You've been invited to join ${organizationName} on Eventure.`,
    `Role: ${assignedRole}`,
    "",
    "Click the link below to activate your account and set your password.",
    "This link expires in 7 days.",
...
function buildEventureInviteHtml(params: EventurePersonnelInviteEmailParams & { link: string }): string {
  const { recipientName, organizationName, assignedRole, link } = params;
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#f4f4f5;font-family:sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f5;padding:40px 0;">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
        <tr><td style="background:#18181b;padding:28px 40px;">
          <h1 style="margin:0;color:#ffffff;font-size:20px;font-weight:700;">Nxt Lvl — Eventure</h1>
        </td></tr>
        <tr><td style="padding:36px 40px;">
          <p style="margin:0 0 16px;font-size:16px;color:#18181b;">Hi ${recipientName},</p>
          <p style="margin:0 0 24px;font-size:15px;color:#444;line-height:1.6;">
            You've been invited to join <strong>${organizationName}</strong> on Eventure.
          </p>
          <div style="background:#f9f9fb;border-radius:6px;padding:16px 20px;margin:0 0 28px;">
            <p style="margin:4px 0;color:#555;">Role: <strong style="color:#18181b;">${assignedRole}</strong></p>
          </div>
