...
function buildSemanticVector(text, dims = 48) {
...
function getTemplateSignature(text, title) {
...
function extractEntities(text) {
    ...
    const dedupe = (items) => [...new Set(items.map((v) => v.trim()).filter(Boolean))].slice(0, 25);
    return {
...
function classifyDocumentTypeAndTopics(title, text) {
    const corpus = `${title}\n${text}`.toLowerCase();
    let bestType = "report";
    let bestTypeScore = 0;
...
function computeFuzzySignature(text) {
    const grams = [...ngrams(text, 4)];
    return grams.sort().slice(0, 60);
}
async function computeFingerprint(text, title, filePath) {
...
function extractExistingVector(searchIndex) {
    if (!searchIndex || typeof searchIndex !== "object")
        return [];
    const vector = searchIndex.embeddingVector;
    if (!Array.isArray(vector))
        return [];
    return vector.filter((value) => typeof value === "number");
}
function extractExistingTemplateSignature(classificationResult) {
    if (!classificationResult || typeof classificationResult !== "object")
        return null;
    const autoLabels = classificationResult.autoLabels;
    if (!autoLabels || typeof autoLabels !== "object")
        return null;
    const signature = autoLabels.templateSignature;
    return typeof signature === "string" ? signature : null;
}
function monthSeriesName(title, type) {
    const lower = title.toLowerCase();
    const monthlyWords = ["monthly", "month", "mtd"];
    if (!monthlyWords.some((word) => lower.includes(word)))
        return null;
    ...
    if (!base)
        return `${type} monthly series`;
    return `${base} monthly series`;
}
function buildStableTaxonomyKey(auto) {
...
async function compareAgainstExistingDocuments(documentId, title, extractedText, month, year, autoLabels, fingerprint) {
    ...
    for (const candidate of candidates) {
        ...
        const candidateTextHash = candidateFingerprint &&
            typeof candidateFingerprint === "object" &&
            typeof candidateFingerprint.exactTextHash === "string"
            ? candidateFingerprint.exactTextHash
            : sha256(normalizeText(candidate.extractedText));
        const candidateFileHash = candidateFingerprint &&
            typeof candidateFingerprint === "object" &&
            typeof candidateFingerprint.fileHash === "string"
            ? candidateFingerprint.fileHash
            : null;
        if (candidateTextHash === fingerprint.exactTextHash ||
...
async function assignDocumentFamily(documentId, title, sourceReference, year, month, autoLabels) {
    ...
    for (const candidate of candidates) {
        ...
        const candidateDocType = (() => {
            if (!candidate.classificationResult || typeof candidate.classificationResult !== "object")
                return "";
            const auto = candidate.classificationResult.autoLabels;
            if (!auto || typeof auto !== "object")
                return "";
            const type = auto.documentType;
        ...
        const existingFamilyId = (() => {
            if (!candidate.searchIndex || typeof candidate.searchIndex !== "object")
                return null;
            const relationships = candidate.searchIndex.relationships;
            if (!relationships || typeof relationships !== "object")
                return null;
            const familyId = relationships.familyId;
...
function buildSearchHelpers(title, text, tags, entities, autoLabels, fingerprint) {
...
async function resolveStableTaxonomy(autoLabels) {
    ...
    for (const doc of docs) {
        if (!doc.classificationResult || typeof doc.classificationResult !== "object")
            continue;
        const taxonomy = doc.classificationResult.taxonomy;
        if (!taxonomy || typeof taxonomy !== "object")
            continue;
        const key = taxonomy.stableCategory;
...
async function processSingleJob() {
    ...
    try {
        ...
        const baseText = extraction.text ||
        ...
        if (autoLabels.documentTypeConfidence < 0.7) {
            reviewReasons.push(`likely ${autoLabels.documentType}, ${Math.round(autoLabels.documentTypeConfidence * 100)}% confidence`);
        }
        if (autoLabels.topicLabels[0] && autoLabels.topicLabels[0].confidence < 0.7) {
            reviewReasons.push(`maybe ${autoLabels.topicLabels[0].label}-related, ${Math.round(autoLabels.topicLabels[0].confidence * 100)}% confidence`);
        }
        ...
        const searchHelpers = buildSearchHelpers(job.document.title, baseText, mergedTags, autoLabels.entities, autoLabels, fingerprint);
        await prisma.document.update({
            where: { id: job.documentId },
            data: {
                ...
                tags: mergedTags,
                keywords: searchHelpers.normalizedKeywords.slice(0, 80),
        ...
        await handleJobFailure({
            ...
            maxAttempts: updatedJob.maxAttempts ??
                MAX_ATTEMPTS,
            errorLog: updatedJob.errorLog ?? null,
        }, err);
    }
}
// ---------------------------------------------------------------------------
